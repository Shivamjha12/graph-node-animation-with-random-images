...
(function (app) {
    function main() {
	console.log("JavaScript file loaded successfully ----------- start");
    ...
    class SvgGraph extends Graph {
        ...
        setOutput(svg) {
            ...
            svg.querySelectorAll("stop")[0].setAttribute("stop-color", "#fff");
            svg.querySelectorAll("stop")[1].setAttribute("stop-color", "#fff");
            return this;
        }
        initSvgGraph() {
            this.initGraph();
        ...
        redrawOutput() {
            ...
            function createSvgElem(tag, attribs) {
                ...
                result.setAttribute("id", `elem_${nextId}`);
            ...
            let arr_img = ['https://xsgames.co/randomusers/assets/avatars/female/67.jpg'];
            function getRandomNumber(min, max) {
                ...
                // Take the floor of the result to get an integer within the specified range
                return Math.floor(randomNumber);
              }
              const imageCache = [];

              // Function to preload images and populate the cache
              function preloadImages() {
                  for (let i = 0; i < 79; i++) {
                      const imageUrl = `https://xsgames.co/randomusers/assets/avatars/female/${i}.jpg`;
                      imageCache.push(imageUrl);
                  }
              }

            preloadImages()
            console.log("Hello ||||||||||||||||||||||||||",imageCache);
            let image_index = 0;
            for (const node of this.nodes)
            {
                ...
                "x": node.posX - (node.radius / 2), // Adjust x position based on the radius of the circle
                "y": node.posY - (node.radius / 2), // Adjust y position based on the radius of the circle
                "width": node.radius, // Set the width of the image to the diameter of the circle
                "height": node.radius, // Set the height of the image to the diameter of the circle
                "href": imageCache[image_index++]
                
                // `https://xsgames.co/randomusers/assets/avatars/female/${image_index++}.jpg`
                }
                );
                if(image_index >= 79){
                    image_index--;
                }
                gElem.append(imageElem);
    ...
    console.log("JavaScript file loaded successfully ----------- end");
